...
| RAG evaluation | LLM Evaluation, RAG Evaluation, Benchmarking, Retrieval Quality | Rebuilt a Pinecone-backed retrieval layer and measured an 80% reduction in incorrect chatbot responses across a 50+ query internal benchmark. |
...
| AI agents | LangGraph router-driven multi-agent assistant architecture for a creative campaign platform | Demonstrates graph routing and separation of responsibilities across specialized agents. |
...
| Production backend engineering | Python/FastAPI services supporting three live applications, including a creative-platform product with 500+ total B2B users during my time | Demonstrates production-scale backend engineering without treating the cumulative user figure as daily active usage. |
